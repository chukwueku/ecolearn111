...
const startMarker = '"ug-statistical": \\`';
const endTokens = '};';

const startIndex = content.indexOf(startMarker);
if (startIndex !== -1) {
    const endBraceIndex = content.lastIndexOf(endTokens);
    if (endBraceIndex !== -1 && endBraceIndex > startIndex) {
        // Replace from the start marker to the 
        const before = content.substring(0, startIndex);
        const after = '\\`\\n};';
        const newContent = before + '"ug-statistical": `\\n' + markdownContent + after;
