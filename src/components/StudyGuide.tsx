  ...
  if (isWideReader) {
    return (
      <div className="fixed inset-0 z-50 bg-slate-50 dark:bg-slate-950 flex flex-col overflow-hidden">
        {/* Fullscreen Header */}
        <div className="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 sm:px-6 py-3 flex items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-3">
            <button 
              onClick={() => {
                setIsWideReader(false);
                setShowFloatingTOC(false);
              }}
              className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white flex items-center gap-1.5 cursor-pointer bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-lg transition-all border border-transparent hover:border-slate-200 dark:hover:border-slate-700"
            >
              <ArrowLeft size={12} />
              Exit Focus
            </button>
            ...
            <div className="min-w-0">
              <span className="text-[9px] font-extrabold uppercase tracking-widest text-sky-600 dark:text-sky-400 block leading-none mb-1">{topic.category}</span>
              <h1 className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200 truncate">{topic.title}</h1>
            </div>
          </div>

          <div className="flex items-center flex-wrap gap-3 sm:gap-4 text-slate-600 dark:text-slate-400">
            {/* Textbook / summary toggle in focus mode */}
            {(topicId === 'ug-international' || topicId === 'ug-econometrics') && (
              <div className="bg-slate-100 dark:bg-slate-950 p-0.5 rounded-xl flex border border-slate-200 dark:border-slate-800/80">
                <button
                  onClick={() => setViewMode('textbook')}
                  className={cn(
                    "px-3 py-1.5 text-[9px] font-bold uppercase tracking-wider rounded-lg transition-all cursor-pointer",
                    viewMode === 'textbook'
            ...
            {/* Column toggler (only in standard view) */}
            {viewMode !== 'textbook' && (
              <button
                onClick={() => setIsDoubleColumn(!isDoubleColumn)}
                className={cn(
                  "px-2.5 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-all border cursor-pointer hidden md:flex items-center gap-1",
                  isDoubleColumn
                    ? "bg-slate-900 dark:bg-sky-600 text-white border-slate-900 dark:border-sky-600 shadow-sm"
                    : "bg-slate-100 dark:bg-slate-950 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-800/80 hover:bg-slate-50 dark:hover:bg-slate-850"
                )}
                title="Toggle Double Column"
              >
                <Columns size={11} />
                <span>2-Col</span>
              </button>
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-[9px] font-bold uppercase tracking-widest text-slate-400">Width</span>
              <div className="bg-slate-100 dark:bg-slate-950 p-0.5 rounded-xl flex border border-slate-200 dark:border-slate-800/80">
                {(['standard', 'wide', 'full'] as const).map((w) => (
                  <button
                    ...
                    className={cn(
                      "px-2 py-1 text-[9px] font-bold uppercase tracking-wider rounded-lg transition-all cursor-pointer",
                      readerWidth === w
            ...
            {/* Font controls */}
            <div className="flex items-center gap-1.5">
              <span className="text-[9px] font-bold uppercase tracking-widest text-slate-400">Text</span>
              <div className="bg-slate-100 dark:bg-slate-950 p-0.5 rounded-xl flex border border-slate-200 dark:border-slate-800/80">
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 md:px-8 py-6 bg-slate-50 dark:bg-slate-950">
          <div className={cn(
            "mx-auto transition-all duration-500",
            readerWidth === 'full' ? "w-full" : readerWidth === 'wide' ? "max-w-5xl" : "max-w-3xl"
          )}>
            {topicId === 'ug-international' && viewMode === 'textbook' ? (
              <React.Suspense fallback={
                ...
                <LazyInternationalEconomicsTextbook isWideReader={isWideReader} readerWidth={readerWidth} readerFontSize={readerFontSize} />
              </React.Suspense>
              ...
              <React.Suspense fallback={
                ...
                <LazyEconometricsStudyGuide isWideReader={isWideReader} readerWidth={readerWidth} readerFontSize={readerFontSize} />
              </React.Suspense>
            ) : (
              <div>
                {loading ? (
                    ...
                    >
                      <AnimatePresence mode="wait" custom={direction}>
                        <motion.div
                          ...
                          animate="animate"
                          exit="exit"
                          className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-8 md:p-12 shadow-xl border-l-[3px] sm:border-l-[6px] border-l-sky-500"
                        >
                          <div className={cn(
                            "markdown-body prose prose-slate dark:prose-invert max-w-none transition-all duration-300",
                            isDoubleColumn ? "lg:columns-2 lg:gap-12 xl:gap-16 [column-fill:auto]" : "",
                            ...
                            <ReactMarkdown
                              ...
                              components={{
                                ...
                                pre: ({ children, node, ...props }: any) => {
                                  ...
                                  return (
                                    <pre className="my-6 p-4 sm:p-6 bg-slate-950 dark:bg-slate-900 border border-slate-800 rounded-xl overflow-x-auto text-xs sm:text-sm font-mono text-slate-100 scrollbar-thin" {...props}>
                                      {children}
                                    </pre>
                                  );
                                },
                                ...
                                blockquote: ({ children }) => (
                                  <blockquote className="my-6 p-4 sm:p-5 bg-sky-50 dark:bg-sky-950/40 border-l-4 border-sky-500 dark:border-sky-400 rounded-r-xl text-slate-900 dark:text-slate-100 text-sm sm:text-base leading-relaxed font-medium shadow-sm">
                                    {children}
                                  </blockquote>
                                ),
                                table: ({ children }) => (
        ...
        {!loading && headingsWithPages.length > 0 && (
          <button
            ...
            className={cn(
              "fixed bottom-8 left-8 z-50 p-4 rounded-full shadow-lg border backdrop-blur transition-all duration-300 flex items-center justify-center cursor-pointer",
              showFloatingTOC
                ? "bg-slate-950 dark:bg-sky-600 text-white border-slate-950 dark:border-sky-600 rotate-90 scale-105"
                : "bg-white/90 dark:bg-slate-900/90 text-slate-800 dark:text-white border-slate-200 dark:border-slate-800 hover:scale-110 shadow-slate-200/50"
            )}
        ...
        {/* Outline Drawer overlay inside focus mode */}
        <AnimatePresence>
          {showFloatingTOC && !loading && headingsWithPages.length > 0 && (
            <>
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 0.4 }}
              ...
              <motion.div
                ...
                className="fixed left-0 top-0 bottom-0 w-80 bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 p-8 z-50 shadow-2xl overflow-y-auto"
              >
                    ...
                    <div className={cn(
                      ...
                      <ReactMarkdown
                        ...
                        components={{
                          ...
                          blockquote: ({ children }) => (
                            <blockquote className="my-6 p-4 sm:p-5 bg-sky-50 dark:bg-sky-950/40 border-l-4 border-sky-500 dark:border-sky-400 rounded-r-xl text-slate-900 dark:text-slate-100 text-sm sm:text-base leading-relaxed font-medium shadow-sm break-inside-avoid">
                              {children}
                            </blockquote>
                          ),
                          table: ({ children }) => (
